-- New households now get neutral default profile names ("Pessoa 1"/"Pessoa 2")
-- instead of "Minha casa"/"Outro perfil", which read like they were written
-- for one specific family rather than any household that signs up. Existing
-- households are untouched — this only changes what gets seeded for new ones.
-- "Pessoa 1"/"Pessoa 2" are already recognized synonyms for Eu/Ela in
-- calc.ts's view-matching, so filters keep working with no client change.
